...
在寻找数的起始位置的时候，scanf回忽略空白字符（space, tab... etc）
注意：因为scanf通常回跳过空白字符，所以除了转换说明，格式串尝尝不包含其他字符。

```c
/*input: 1-20.3-4.0e3ENTER*/
scanf("%d%d%f%f", &i, &j, &x, &y);
```
 - 读取1然后向后读到"-",不能出现在整数中，所以把1存入变量i
 - 随后因为整数不能包含".", 把-20存入变量j中
 - 同理，0.3 存入x中
 - 最后$-4.0 \times 10^3$ 存入 y
